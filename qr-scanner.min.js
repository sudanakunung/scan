class e {
  ...
  static async scanImage(a, b, c, d, f = !1, h = !1) {
    ...
    try {
      ...
      if (c instanceof Worker) {
        let g = c;
        b || g.postMessage({ type: "inversionMode", data: "both" });
        r = await new Promise((l, v) => {
          let w, u, t;
          u = (q) => {
            "qrResult" === q.data.type &&
              (g.removeEventListener("message", u),
              g.removeEventListener("error", t),
              clearTimeout(w),
              null !== q.data.data
                ? l({
                    data: q.data.data,
                    cornerPoints: e._convertPoints(q.data.cornerPoints, m),
                  })
          ...
          t = (q) => {
            ...
            g.removeEventListener("error", t);
            clearTimeout(w);
            v("Scanner error: " + (q ? q.message || q : "Unknown Error"));
          };
          g.addEventListener("message", u);
          g.addEventListener("error", t);
          w = setTimeout(() => t("timeout"), 1e4);
          let x = k.getImageData(0, 0, d.width, d.height);
          g.postMessage({ type: "decode", data: x }, [x.data.buffer]);
        });
      } else
  ...
  static async createQrEngine(a) {
    ...
    return !e._disableBarcodeDetector &&
      ...
      : import("./qr-scanner-worker.min.js").then((b) => b.default);
  }
  _onPlay() {
  ...
  _updateOverlay() {
    requestAnimationFrame(() => {
      if (this.$overlay) {
        ...
        var [v, w] = n.objectPosition.split(" ").map((t, x) => {
          const q = parseFloat(t);
          return t.endsWith("%") ? ((x ? f - l : d - g) * q) / 100 : q;
        });
        n = this._scanRegion.width || b;
        r = this._scanRegion.height || c;
  ...
  static async _postWorkerMessage(a, b, c) {
    a = await a;
